$(document).ready(function () {
  ...
  $("body").on("click", ".cross-peer", function (e) {
    $(this).closest(".peer-input-field .intension-container").last().remove();
  });
  ...
  $("body").on("click", ".save-user", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data == "false") {
          alert(
            "Your sub description total does not match with main description amount"
          );
        } else if (data == "already_user") {
          alert(
            "This bill is already created for particular address,country,user,type of payment and for particular date"
          );
        } else if (data == "already_company") {
          alert(
            "This bill is already created for particular address,country,type of payment and for particular date"
          );
        } else {
          alert("Submitted");
...
$(document).ready(function () {
  var cmp_html =
    '<div class="pymt_cmpy" style="margin-top: 20px;"><span class="profile-info myClass1">Type of payment:<select id="cmp_list" class="selectbox" name="Select_Box" style="margin-bottom: -5px;"></select></span></div>';

  var html =
    '<div id="box" style="margin-bottom:24px 0px -14px -1px;"><span class="profile-info myClass">User:<select  id="user_list" class="selectbox" name="Select_B1"></select></span><br><br><span class="profile-info myClass">Type of payment:<select  id="cmp_list" class="selectbox" name="Select_B2"></select></span></div>';
